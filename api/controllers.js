...
const Feed = async (req, res) => {
  ...
  // Menyimpan kata kunci dan tanggal dalam objek
  let kw = [];
  ...
  // Menyediakan header XML untuk feed RSS
...
\t<generator><![CDATA[${appConfig["title"]}]]></generator>\n`);
  ...
  // Menambahkan item-item berdasarkan kata kunci dan tanggal yang relevan
  for (let i in kw) {
    let pubDate = new Date();  // Default date
    if (dates[kw[i]]) {
      pubDate = new Date(dates[kw[i]]);  // Mengambil tanggal yang relevan
    }
    res.write(`\t<item>
\t\t<title><![CDATA[${ucwords(kw[i])}]]></title>
\t\t<description><![CDATA[Read Or Download ${ucwords(kw[i])} at ${appConfig["title"]}]]></description>
...
const SitemapXML = async (req, res) => {
  ...
  res.write(
    `\t<url>\n\t\t<loc><![CDATA[${originUrl}/]]></loc>\n\t\t<lastmod><![CDATA[${new Date().toISOString()}]]></lastmod>\n\t</url>\n`
  );

  // Menambahkan setiap URL dengan tanggal yang sesuai
  for (let i in kw) {
    if (kw[i] != "") {
      let time = dates[kw[i]] ? new Date(dates[kw[i]]) : new Date();  // Mengambil tanggal yang relevan
      res.write(
        `\t<url>\n\t\t<loc><![CDATA[${originUrl}/${kw[i]
          .replace(/\s+/g, " ")
          .replace(/\s/g, "-")}/]]></loc>\n\t\t<lastmod><![CDATA[${time.toISOString()}]]></lastmod>\n\t</url>\n`
      );
    }
  }
...
const Pagination = async (req, res) => {
  ...
  // Mendapatkan ID halaman dari URL
  let id = parseInt(req.params.id);
  if (isNaN(id) || id <= 0) {
    res.redirect("/");  // Jika ID tidak valid, redirect ke halaman utama
    return;
  }

  // Menghitung start dan end untuk pemotongan data kata kunci per halaman
  let start = (id - 1) * 27;
  let end = start + 27;
  let data = kw.slice(start, end);

  // Jika data tidak ditemukan (halaman kosong), tampilkan halaman 404
  if (data.length === 0) {
  ...
  // Mengatur informasi halaman di config
  ...
  appConfig["prevPage"] = id > 1 ? id - 1 : null;  // Halaman sebelumnya
  appConfig["nextPage"] = end < kw.length ? id + 1 : null;  // Halaman berikutnya
  appConfig["firstPage"] = 1;  // Halaman pertama
  appConfig["lastPage"] = Math.ceil(kw.length / 27);  // Halaman terakhir
  appConfig["image"] = appConfig["favicon"];
  appConfig["baseUrl"] = originUrl;
  appConfig["fullUrl"] = fullUrl;

  // Menyajikan informasi pagination
  appConfig["pagination"] = {
    prev: appConfig["prevPage"],
    next: appConfig["nextPage"],
    first: appConfig["firstPage"],
    last: appConfig["lastPage"],
  };

  // Menyajikan data tanggal yang sesuai dengan kata kunci
  appConfig["kwDates"] = kwDates; // Menyertakan tanggal untuk setiap kata kunci

  // Menyajikan halaman dengan layout
  res.type("html");
...
const Homepage = async (req, res) => {
  ...
  // Menentukan data yang akan ditampilkan (misalnya 27 kata kunci pertama)
  let data = kw.slice(0, 27);

  // Mengatur konfigurasi halaman
  appConfig["typePage"] = "homepage";
  appConfig["type"] = "website";
  appConfig["titlePage"] = "Homepage";
  appConfig["kw"] = kw;
  appConfig["dataKw"] = data;
  appConfig["currentPage"] = 1;  // Halaman pertama
  appConfig["prevPage"] = null;  // Tidak ada halaman sebelumnya
  appConfig["nextPage"] = kw.length > 27 ? 2 : null;  // Jika ada lebih dari 27 kata kunci, tampilkan halaman berikutnya
  appConfig["image"] = appConfig["favicon"];
  appConfig["baseUrl"] = originUrl;
  appConfig["fullUrl"] = fullUrl;

  // Menyertakan tanggal untuk setiap kata kunci
  appConfig["kwDates"] = kwDates;

  // Menampilkan layout halaman dengan konfigurasi
  res.type("html");
  res.write(await Layout(appConfig));
  res.send();
};
